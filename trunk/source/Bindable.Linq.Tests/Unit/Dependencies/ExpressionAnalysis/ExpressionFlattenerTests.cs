...
namespace Bindable.Linq.Tests.Unit.Dependencies.ExpressionAnalysis
{
    ...
    public sealed class ExpressionFlattenerTests : TestFixture
    {
        ...
        private void TestExpression<TDelegate>(Expression<TDelegate> expression, params ExpressionType[] expectedExpressionTypes)
        {
            ...
            if (failed)
            {
                Assert.Fail("Expression \"{0}\" should have found expressions \"{1}\", but instead found expressions of type \"{2}\"", expression, expectedExpressionTypes.Select(xt => xt.ToString()).ConcatStrings(", "), flattener.Expressions.Select(x => x.NodeType.ToString()).ConcatStrings(", "));
            }
        }
